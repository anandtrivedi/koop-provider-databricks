/**
 * Create Test Tables in Azure Databricks Workspace
 *
 * This creates the 4 test layers in your Azure workspace:
 * - atrivedi.geospatial.koop_test_cities (50 cities)
 * - atrivedi.geospatial.koop_test_highways (5 highways)
 * - atrivedi.geospatial.koop_test_states (5 states)
 * - atrivedi.geospatial.koop_test_parks (10 parks)
 */

const { DBSQLClient } = require('@databricks/sql');
require('dotenv').config({ path: require('path').join(__dirname, '../../.env') });

// Azure Databricks connection config
const config = {
...
async function createTestLayers() {
  ...
  console.log('✅ Connected to Azure Databricks\n');

  try {
    const session = await connection.openSession();

    // 1. CREATE CATALOG AND SCHEMA (if they don't exist)
    console.log('📁 Creating catalog and schema...');
    try {
      await session.executeStatement('CREATE CATALOG IF NOT EXISTS atrivedi');
      console.log('   ✅ Catalog: atrivedi');
    } catch (e) {
      console.log('   ℹ️  Catalog already exists or no permission to create');
    }

    try {
      await session.executeStatement('CREATE SCHEMA IF NOT EXISTS atrivedi.geospatial');
      console.log('   ✅ Schema: atrivedi.geospatial');
    } catch (e) {
      console.log('   ℹ️  Schema already exists or no permission to create');
    }

    // 2. DROP existing tables
    console.log('\n🗑️  Dropping existing test tables...');
    const tablesToDrop = [
    ...
    // 3. CREATE CITIES TABLE (Points - 50 cities)
    ...
    console.log('📊 Summary:');
    console.log('   • koop_test_cities: 10 cities (Points)');
    console.log('   • koop_test_highways: 2 highways (LineStrings)');
    console.log('   • koop_test_states: 2 states (Polygons)');
    console.log('   • koop_test_parks: 3 national parks (Points)');
    console.log('\n🌐 Test these layers at:');
    console.log('   https://koop-databricks.onrender.com/databricks/rest/services/atrivedi.geospatial.koop_test_cities/FeatureServer/0');
    console.log('   https://koop-databricks.onrender.com/databricks/rest/services/atrivedi.geospatial.koop_test_highways/FeatureServer/0');
    console.log('   https://koop-databricks.onrender.com/databricks/rest/services/atrivedi.geospatial.koop_test_states/FeatureServer/0');
    console.log('   https://koop-databricks.onrender.com/databricks/rest/services/atrivedi.geospatial.koop_test_parks/FeatureServer/0');

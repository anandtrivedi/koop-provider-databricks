/**
 * Create Test Tables in Databricks Community Edition
 */
...
async function createTestLayers() {
  ...
  try {
    ...
    // Note: Community Edition uses 'default' schema, no custom catalogs
    console.log('📁 Using default schema...');

    // Drop existing tables
    console.log('\n🗑️  Dropping existing test tables...');
    const tablesToDrop = [
      'default.koop_test_cities',
      'default.koop_test_highways',
      'default.koop_test_states',
      'default.koop_test_parks'
    ];
    ...
    await session.executeStatement(`
      CREATE TABLE default.koop_test_cities (
        objectid INT,
        city_name STRING,
        state STRING,
        population BIGINT,
    ...
    for (const city of cities) {
      await session.executeStatement(`
        INSERT INTO default.koop_test_cities VALUES (
    ...
    await session.executeStatement(`
      CREATE TABLE default.koop_test_highways (
        objectid INT,
        highway_name STRING,
    ...
    for (const hwy of highways) {
      await session.executeStatement(`
        INSERT INTO default.koop_test_highways VALUES (
    ...
    await session.executeStatement(`
      CREATE TABLE default.koop_test_states (
        objectid INT,
        state_name STRING,
        abbreviation STRING,
        population BIGINT,
    ...
    for (const state of states) {
      await session.executeStatement(`
        INSERT INTO default.koop_test_states VALUES (
    ...
    await session.executeStatement(`
      CREATE TABLE default.koop_test_parks (
        objectid INT,
        park_name STRING,
        state STRING,
        area_acres INT,
    ...
    for (const park of parks) {
      await session.executeStatement(`
        INSERT INTO default.koop_test_parks VALUES (
    ...
    console.log('📊 Summary:');
    console.log('   • default.koop_test_cities: 10 cities');
    console.log('   • default.koop_test_highways: 2 highways');
    console.log('   • default.koop_test_states: 2 states');
    console.log('   • default.koop_test_parks: 3 parks');
    console.log('\n🌐 Test URLs:');
    console.log('   https://koop-databricks.onrender.com/databricks/rest/services/default.koop_test_cities/FeatureServer/0');
